...
- CMHF-05: two directions per control for user comparison — ReleaseYearControl A (segmented mode tabs + year inputs) / B (dual-handle range slider + mode chips); LikedDateTimeline A (year-preset pills + mode tabs + date inputs) / B (horizontal timeline + draggable handles). Out-of-bounds SAVED values: clamp handle POSITION for display only (0–100%), keep the real value in inputs/aria/stored filter. Year presets persist fixed `YYYY-01-01`..`YYYY-12-31` UTC; through-today persists `end.kind:"today"`. A Ladle stub for `getPlaylistMatchFilterOptions` was added to `src/__mocks__/playlists.functions.stub.ts` (committed with CMHF-08).
- CMHF-08: options payload returns BOTH detected (source detected, count desc first) and catalog-only (source catalog, alphabetical) languages in one response so CMHF-14 needs no second round-trip; uncataloged detected codes excluded + logged once per code. Eligibility = `getEntitledDataEnrichedSongIds` (same as match refresh). Added `playlistMatchFilterOptionsQueryOptions(accountId)` + `playlistKeys.filterOptions` for CMHF-14. Aggregation via batched PostgREST queries (no raw SQL), parallelized.
- CMHF-12: `effectiveExclusionSet` built once; a single `exclusionSetArg` (size>0 ? set : undefined) is passed to BOTH `matchBatch` and `writeMatchSnapshot` (so the snapshot hash reflects filter changes and the two can't diverge). Base + filter exclusion loads run via `Promise.all`. `loadMatchFilterExclusions` hardened to truly never throw (eval loop wrapped → degrades to empty + `degraded.filterMetadata`); degraded flag logged as a fresh object to avoid mutation fragility.
- Note: pre-existing `z.string().uuid()` deprecation notices in `playlists.functions.ts` (lines 98/197/364/434) are non-fatal (tsgo exit 0, biome ignores) and out of scope for this feature; left as-is to avoid unrelated churn.
- CMHF-06: CONVERGED production picks (for CMHF-13) = LanguagePickerCombobox + ReleaseYearControlA + LikedDateTimelineA — chosen for drawer-width friendliness (always-visible list / explicit mode tabs work at narrow widths; B variants' drag sliders need more horizontal room). Non-chosen directions kept as standalone "Match Filters/*" stories for the user to compare/swap. `AdvancedFiltersAssembly` wrapper threads one draft + options + optionsState into the panel slots.
- CMHF-06: `SpotlightPanel` gained an optional non-breaking `advancedFiltersSlot?: ReactNode` (threads into WritingSurface's `advancedFilters` slot) — production callers unaffected. Deferred to the user's Ladle iteration: always-open combobox keeps `aria-expanded="true"`, and the narrow-width drawer story uses the Ladle width addon (no programmatic constraint).
- CMHF-07: combined save uses `parseSaveMatchFilters` (strict) → reject-before-write (all-or-nothing); `updatePlaylistMatchConfig` does a single 3-field update scoped by account_id; non-fatal invalidation failure after write logs + still returns normalized values. Old `savePlaylistMatchIntent`/`savePlaylistGenrePills` kept (CMHF-15 migrates callers). matchIntent normalized via `.trim()` only (internal whitespace preserved); genres via existing `sanitizeGenrePills`.
