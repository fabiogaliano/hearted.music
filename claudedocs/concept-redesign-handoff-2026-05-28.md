...
├── concept-migration.ts          ← NEW (S3): legacy → concept-draft transformer (pure fn)
...
scripts/voice-audit/
├── transform-legacy-exemplars.ts ← NEW (S3): runs the transformer over gold
└── exemplars-v14-draft/*.json    ← NEW (S3): scaffolded drafts (lens/contradiction/take stubbed)
