...
| `name`        | TEXT NOT NULL        | —                               |
| `artists`     | TEXT[] NOT NULL      | Was `artist` (single string)    |
| `album_name`  | TEXT                 | Was `album`                     |
| `album_id`    | TEXT                 | NEW                             |
| `image_url`   | TEXT                 | Renamed from `album_image_url`  |
| `duration_ms` | INTEGER              | NEW                             |
| `popularity`  | INTEGER              | NEW                             |
| `preview_url` | TEXT                 | NEW                             |
| `genres`      | TEXT[]               | NEW (moved from `track_genres`) |
| `created_at`  | TIMESTAMPTZ          | —                               |
| `updated_at`  | TIMESTAMPTZ          | NEW                             |

**Indexes**: `spotify_id` (unique), `isrc`, `genres` (GIN)
...
**Indexes**: `account_id`, `spotify_id`, `(account_id) WHERE is_destination = true`
...
| `id`                  | UUID            | —                                                                                    |
| `account_id`          | UUID FK         | Was `user_id` INTEGER                                                                |
| `type`                | job_type ENUM   | Values: sync_liked_songs, sync_playlists, song_analysis, playlist_analysis, matching |
| `status`              | job_status ENUM | Values: pending, running, completed, failed                                          |
...
| Store artists as array?            | ✅ Store as TEXT[]                                                    |
