...
This is **not** the progressive match-feed architecture. `docs/architecture/matching/progressive-match-feed.md` now says that refactor is future/gated; this plan fixes a concrete per-card read/render bottleneck without changing snapshot or queue publication semantics.
