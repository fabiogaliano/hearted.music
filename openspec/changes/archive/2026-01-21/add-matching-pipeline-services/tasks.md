...
- [x] 0.1 Add `LASTFM_API_KEY` to `src/env.ts` (optional, graceful degradation if missing)
- [x] 0.2 Verify `song.genres` column exists in schema (TEXT[] with max 3 elements)
- [x] 0.3 Verify `playlist_profile` table exists with required columns
...
- [x] 1.1.1 Create `src/lib/integrations/lastfm/service.ts`
  - Port `LastFmService` class from `old_app/lib/services/lastfm/LastFmService.ts`
  - Use `ConcurrencyLimiter` from `@/lib/shared/utils/concurrency.ts` (5 concurrent, 200ms interval)
...
- [x] 1.1.2 Create `src/lib/integrations/lastfm/whitelist.ts`
...
- [x] 1.1.3 Create `src/lib/integrations/lastfm/normalize.ts`
...
- [x] 1.1.4 Create `src/lib/integrations/lastfm/types.ts`
...
- [x] 1.2.1 Create `src/lib/capabilities/genre/service.ts`
...
- [x] 1.2.2 Add genre queries to `src/lib/data/song.ts`
...
- [x] 1.2.3 Create `src/lib/shared/errors/external/lastfm.ts`
...
- [x] 2.1.1 Create `src/lib/capabilities/profiling/service.ts`
...
- [x] 2.1.2 Create `src/lib/capabilities/profiling/types.ts`
...
- [x] 2.1.3 Port centroid calculation utilities
...
- [x] 2.1.4 Use playlist profile queries in `src/lib/data/vectors.ts`
...
- [x] 2.2.1 Create `src/lib/integrations/reccobeats/service.ts`
  - Port `ReccoBeatsService` from `old_app/lib/services/reccobeats/ReccoBeatsService.ts`
  - Use `ConcurrencyLimiter` from `@/lib/shared/utils/concurrency.ts` (5 concurrent, 50ms interval)
...
- [x] 2.2.2 Create `src/lib/integrations/audio/service.ts`
...
- [x] 2.2.3 Create `src/lib/shared/errors/external/reccobeats.ts`
  - `ReccoBeatsRateLimitError` - 429 from API
  - `ReccoBeatsNotFoundError` - No features found
  - `ReccoBeatsApiError` - Other API errors

- [x] 2.2.4 Integrate audio features service into profiling
...
- [x] 3.1.1 Create `src/lib/ml/embedding/extractors.ts`
...
- [x] 3.1.2 Create `src/lib/ml/embedding/hashing.ts`
...
- [x] 3.1.3 Create `src/lib/ml/embedding/versioning.ts`
...
- [x] 3.2.1 Create `src/lib/capabilities/matching/semantic.ts`
...
- [x] 3.3.1 Create `src/lib/capabilities/matching/config.ts`
...
- [x] 3.4.1 Create `src/lib/capabilities/matching/service.ts`
...
- [x] 3.4.2 Create `src/lib/capabilities/matching/types.ts`
...
- [x] 3.5.1 Create `src/lib/capabilities/matching/cache.ts`
