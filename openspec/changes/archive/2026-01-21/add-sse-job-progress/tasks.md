...
- [x] 0.1 Confirm API route naming pattern uses `src/routes/api.{name}.tsx` (see `openspec/project.md`)
- [x] 0.2 Verify `data/jobs.ts` has all needed queries (getJobById, updateJobProgress)
- [x] 0.3 Review existing job types in `data/jobs.ts` match SSE event needs
...
- [x] 1.1.1 Create `src/lib/jobs/progress/types.ts`
...
- [x] 1.2.1 Create `src/lib/jobs/progress/emitter.ts`
...
- [x] 1.3.2 Implement SSE stream in endpoint
...
- [x] 1.3.4 Add initial state push
...
- [x] 1.3.5 Handle terminal states
...
- [x] 2.1.1 Create `src/lib/hooks/useJobProgress.ts`
...
- [x] 2.1.2 Implement EventSource connection
...
- [x] 2.1.3 Integrate with TanStack Query
...
- [x] 2.1.4 Add reconnection state tracking
...
- [x] 3.1.1 Create `src/lib/jobs/progress/helpers.ts`
...
- [x] 3.2.1 Update `src/lib/capabilities/sync/orchestrator.ts`
...
- [x] 3.3.1 Update `src/lib/capabilities/analysis/pipeline.ts`
  - Import SSE helpers
...
- [x] 3.4.1 Update `src/lib/capabilities/matching/service.ts` (when created)
  - Import SSE helpers
