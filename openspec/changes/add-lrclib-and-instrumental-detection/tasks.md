...
- [x] 1.1 Add `src/lib/domains/enrichment/lyrics/providers/lrclib.ts`: a client
...
- [x] 1.3 Define the shared `LyricsOutcome` type
      (`lyrics | instrumental | not_found`, with source
      `lrclib | genius | genius_page` + confidence) in
      `types/lyrics.types.ts`.
- [x] 1.4 Thread `albumName` and `durationSec` through the lyrics fetch path:
...
- [x] 2.1 In `src/lib/domains/enrichment/lyrics/service.ts` `fetchLyrics`,
...
- [x] 2.2 Snapshot-test the marker detection against a captured
...
- [x] 3.1 In `src/lib/domains/enrichment/lyrics/service.ts`, make
...
- [x] 3.2 Apply the spurious-match override: LRCLIB `instrumental: true` wins
...
- [x] 3.3 Update `createLyricsService` wiring and any callers consuming the old
      `string | NoLyricsError` shape to the new `LyricsOutcome`.

## 4. Persistence

- [x] 4.1 Add a migration in `supabase/migrations/` extending `song_lyrics`
      ...
      option A.)
- [x] 4.2 In `src/lib/domains/enrichment/lyrics/queries.ts`, always upsert a
      row after a fetch attempt recording `fetch_status` + `source`; expose a
      read that distinguishes "no row" from `not_found`.
- [x] 4.3 Run the migration locally and verify the column + a sample of each
      `fetch_status` via the supabase-local workflow.

## 5. Content-Type Classifier (analysis routing)

- [x] 5.1 In `src/lib/domains/enrichment/content-analysis/song-analysis.ts`,
      ...
      no vote.
- [x] 5.2 Add the curated instrumental genre keyword set (excluding generic
      electronic tags) as a single source of truth (e.g.
      `content-analysis/instrumental-genres.ts`).
- [x] 5.3 Route `analyzeSong`: lyrical → lyrical prompt; instrumental →
...
- [x] 7.1 In `content-analysis/song-batch-analysis.ts` and
...
- [x] 7.2 In `enrichment-pipeline/failure-policy.ts`, add the
...
- [x] 8.2 Integration-test the recreated selector against local fixtures: a
...
- [x] 9.1 Extend the `enrichment_stopped` reason union with `"blocked"` in
      `library-processing/types.ts` and `changes/enrichment.ts`.
- [x] 9.2 In `library-processing/runner.ts` (and `worker/execute.ts` as
...
- [x] 9.3 Test: a blocked chunk leaves the workflow stale without re-ensuring a
...
- [x] 10.2 In `SongDetailPanelSurface.tsx`, render the instrumental read
      (headline / compound_mood / sonic_texture / mood_description),
      brand-voiced and sound-first.
- [x] 10.3 Split `UnreadState`: a distinct, honest "lyrics unavailable" copy
...
- [x] 11.1 Remove instrumentalness as a routing input everywhere except the
...
- [x] 12.1 Provider tests for LRCLIB (instrumental: true, lyrics hit, 404,
      duration-mismatch search fallback) and the provider-order/override logic
      in LyricsService (`__tests__/` co-located).
- [x] 12.2 Classifier tests covering the precedence with the real cases: Need
...
- [x] 12.3 Adapter test: an instrumental-shaped analysis row yields a
      renderable instrumental read (not `read = null`).
- [x] 12.4 Incident convergence check against the local DB: account
      ...
      of looping.
- [x] 12.5 `bun run test`, `bun run typecheck`, and `bunx biome check` green;
      `openspec validate add-lrclib-and-instrumental-detection --strict`
      passes.

## 13. Resolved-Unknown Presentation (D2 follow-on)

Realizes the song-content-type spec scenario "Analyzing versus resolved" for
the unknown case: a cleanly-resolved unknown song (lyrics-fetch settled, no
read) must show the "lyrics unavailable" state, not "Listening" forever.

- [x] 13.1 Migration recreating the `liked_song_decorated` view to surface the
      latest `song_lyrics.fetch_status` as a new column (e.g.
      `content_fetch_status`), preserving every existing column/expression and
      adding only the new one. Apply via CLI migrate up (never reset); update
      `database.types.ts` for the view's new column.
- [x] 13.2 Thread the new column through the liked-song page query/RPC →
      `SongDetail` → `song-detail-adapter.ts` → `SongDetailPanelSurface.tsx`,
      and refine the UnreadState predicate so: read present → read; no read but
      lyrics-fetch settled to no-read (`not_found`) → "No words yet"; otherwise
      "Listening" only while genuinely pre-resolution.
- [x] 13.3 Test: an unknown song (fetch settled `not_found`, no read) renders
      "No words yet" (not "Listening"); a genuinely in-flight song still renders
      "Listening"; lyrical/instrumental reads unchanged.
