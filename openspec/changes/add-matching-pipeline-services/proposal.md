...
| ------------------------------------------------------ | -------------------------------------- | ----- | --------------------------------------- |
| `old_app/lib/services/lastfm/LastFmService.ts`         | `src/lib/services/lastfm/service.ts`   | 311   | Last.fm API client with rate limiting   |
| `old_app/lib/services/lastfm/utils/genre-whitelist.ts` | `src/lib/services/lastfm/whitelist.ts` | 469   | 469-genre canonical taxonomy            |
| `old_app/lib/services/lastfm/utils/normalize.ts`       | `src/lib/services/lastfm/normalize.ts` | 81    | Artist/album name normalization         |
| `old_app/lib/services/genre/GenreEnrichmentService.ts` | `src/lib/services/genre/service.ts`    | 477   | DB-first genre enrichment orchestration |

**Key Adaptations:**
- Replace old logging/metrics with v1 patterns (`better-result` errors)
- Use `ConcurrencyLimiter` from `@/lib/utils/concurrency.ts`
...
| Source File                                                  | Target Location                          | Lines | Purpose                                 |
| ------------------------------------------------------------ | ---------------------------------------- | ----- | --------------------------------------- |
| `old_app/lib/services/profiling/PlaylistProfilingService.ts` | `src/lib/services/profiling/service.ts`  | 770   | DB-first profile computation            |
| `old_app/lib/services/reccobeats/ReccoBeatsService.ts`       | `src/lib/services/reccobeats/service.ts` | 226   | ReccoBeats audio features API           |
| `old_app/lib/services/audio/AudioFeaturesService.ts`         | `src/lib/services/audio/service.ts`      | 45    | Audio features backfill + normalization |

**Key Adaptations:**
- Use existing `EmbeddingService` from `src/lib/services/embedding/service.ts`
...
| Source File                                                 | Target Location                            | Lines | Purpose                         |
| ----------------------------------------------------------- | ------------------------------------------ | ----- | ------------------------------- |
| `old_app/lib/services/matching/matching-config.ts`          | `src/lib/services/matching/config.ts`      | 125   | Algorithm weights & thresholds  |
| `old_app/lib/services/semantic/SemanticMatcher.ts`          | `src/lib/services/matching/semantic.ts`    | 306   | Theme/mood similarity           |
| `old_app/lib/services/vectorization/analysis-extractors.ts` | `src/lib/services/embedding/extractors.ts` | 354   | Text extraction from analysis   |
| `old_app/lib/services/vectorization/hashing.ts`             | `src/lib/services/embedding/hashing.ts`    | 327   | Content hashing for cache keys  |
| `old_app/lib/services/matching/MatchingService.ts`          | `src/lib/services/matching/service.ts`     | 1493  | Core multi-factor scoring       |
| `old_app/lib/services/matching/MatchCachingService.ts`      | `src/lib/services/matching/cache.ts`       | 534   | Cache-first match orchestration |
...
- `src/lib/services/embedding/service.ts` - Song embedding generation
- `src/lib/services/deepinfra/service.ts` - DeepInfra API (embeddings + reranker)
- `src/lib/services/reranker/service.ts` - Cross-encoder reranking
...
### Affected Code
- `src/lib/services/` - New service modules
- `src/lib/services/embedding/` - Add extractors and hashing utilities
