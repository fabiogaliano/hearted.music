...
| `src/lib/services/job-progress/emitter.ts` | In-memory event emitter for SSE                |
| `src/lib/services/job-progress/types.ts`   | SSE event types (including item status events) |
...
| `src/lib/services/job-lifecycle.ts` | Job state transitions      |
...
- `src/lib/services/` - New job progress service
- `src/lib/hooks/` - New client hook
- Services that update job progress will emit SSE events:
  - `src/lib/services/sync/orchestrator.ts`
  - `src/lib/services/analysis/pipeline.ts`
  - `src/lib/services/matching/service.ts` (Phase 4e)
