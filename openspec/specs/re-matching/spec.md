...
> Re-match operation that runs matching on all data-enriched songs when playlist profiles change, without re-running the full enrichment pipeline.

---

## Purpose

TBD - Define the re-match capability that detects playlist profile changes and re-evaluates all enriched songs against updated playlist profiles without repeating data enrichment stages.
