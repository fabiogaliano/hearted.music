# Matching Session Specification

> Server-side session initialization, single-song loading with offset pagination, prefetch strategy, and session lifecycle tracking.

## Purpose

Define how the matching page initializes a session context, loads songs one at a time via offset, prefetches upcoming songs for instant transitions, and batch-clears newness on session end.
