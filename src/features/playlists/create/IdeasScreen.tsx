/**
 * IdeasScreen — beat 1 of creation, the route body for /playlists/new.
 *
 * The entrance owns no draft state: IdeasBoard hands back a chosen idea (or
 ...
 * now distinct routes, so "up a level" is a real route hop — IdeasBoard's back
...
export function IdeasScreen({ billingState }: IdeasScreenProps) {
	...
	// whether a typed intent survives into the studio — ideas are structured
	...
	const handleSeed = useCallback(
		(idea: ResolvedIdeaVM | null, intentText: string) => {
			const carriedIntent = isIntentEligible ? intentText : "";
			const studioSeed = buildStudioSeed(idea, carriedIntent);
