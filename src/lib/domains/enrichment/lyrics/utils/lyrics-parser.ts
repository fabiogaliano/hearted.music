...
export function parseLyrics(html: string): LyricsSection[] {
	...
	for (const container of containers) {
		const lines = extractLinesFromContainer(container);
		allLines.push(...lines);
	}

	// Parse into sections
	return buildSections(allLines);
}

function extractLinesFromContainer(container: Element): LineData[] {
...
function buildSections(lines: LineData[]): LyricsSection[] {
