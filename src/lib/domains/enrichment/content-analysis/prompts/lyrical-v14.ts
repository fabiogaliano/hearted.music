...
export const lyricalV14: PromptVersion = {
	...
	kind: "lyrical",
	notes:
		"First prompt for the redesigned { read } content model (read-schema.ts SongReadSchema). Iterates from v13, not a rewrite: v13's HOW TO WRITE voice block and puffery ban carry over verbatim because they are proven at temperature 0.3 and field-agnostic. What changed: (1) the eight old fields (headline/compound_mood/mood_description/interpretation/themes/journey/key_lines/sonic_texture) are replaced by the read fields (image/lens/tension/take/contradiction/arc/lines/texture); (2) a new lens field is the thesis, generated first so take/arc/lines read as evidence for one claim instead of eight parallel ones; (3) the schema-overprescription diagnostic's findings are encoded as generation rules: tension is a qualified emotion not a paradox, take is elastic 1-3 sentences matched to depth, arc floors at 2 with a mood that may repeat, lines floors at 1, contradiction is null when the song holds none, and a surface-true song is explicitly granted a surface-true read; (4) foreign-language lines quote the original with a parenthetical English gloss. The lens forms, families, and kill-checks compress concept-lens-vocabulary.md. NOT YET ACTIVE: song-analysis.ts still parses against the old SongAnalysisLyricalSchema, so v14 ships as a selectable draft (getLyricalPrompt('14')) with v13 left active; the generation-path + jury cutover to SongReadSchema is Session 5/6. Intended to run at temperature 0.3, the proven low-variance setting.",
