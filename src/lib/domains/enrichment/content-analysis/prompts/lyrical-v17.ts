...
// v17 = v16 verbatim plus two runtime-injected slots: {example} and {annotations}. No gold text
// is baked into this file — the slots are filled at call time. In eval (regen.ts) {example} is a
// leave-one-out few-shot block (a song never sees its own gold) and {annotations} is that song's
// own vote-gated grounding annotations (no leakage: a song is allowed to see its own annotations).
// In prod the same slots are filled by the caller; SongAnalysisService only plumbs prebuilt
// strings through (it never fetches golds or annotations itself). Both slots are OPTIONAL — an
// empty string is valid and collapses to whitespace. Registered but NOT active: production still
// ships v13. See claudedocs/06-block1-implementation-plan.md WP1.
export const lyricalV17: PromptVersion = {
	version: "17",
	kind: "lyrical",
	notes:
		"v16 + runtime-injected {example} and {annotations} slots. {example} carries 1–2 worked gold reads as a few-shot block, selected leave-one-out so a song never sees its own gold (regen.ts). {annotations} carries the song's vote-gated (>15) grounding annotations from the same shared selector the grounding judge uses, so eval and prod see identical grounding; annotations are NOT leave-one-out (a song seeing its own annotations is not leakage). No exemplar JSON is pasted into this file. Both slots are optional and empty-safe. Everything else carries over from v16 verbatim. Intended to run at temperature 0.3.",
