...
// line against this ("Say what something is. Don't say what it 'isn't'…") but it showed only the in-clause
// form and got buried, so the cross-sentence form sailed through on essentially every candidate. The single
// change here REPLACES that line with a behavioral, example-driven ban covering the move in all its forms
// (in-clause, cross-sentence, mirrored parallelism), using the actual cited candidate failures as the Wrong
// examples and INVENTED Right rewrites (this file's NO-GOLD-BAKED-IN rule holds). No other line of v17 is
// touched. ACTIVE stays v13. Run at temperature 0.3.
//
...
export const lyricalV20: PromptVersion = {
	version: "20",
	kind: "lyrical",
	notes:
		"v17 + ONE register edit (Phase-4 hypothesis H2): replaces v17's weak anti-antithesis line with a behavioral, example-driven ban on the thesis-pivot ('not X, it is Y') in all its forms — in-clause, cross-sentence, mirrored parallelism — the single most-cited essayistic-register tell across both gemini-2.5-flash and -pro. Wrong examples are real candidate failures; Right rewrites are invented (NO-GOLD-BAKED-IN held). Everything else identical to v17. Branches from v17, not the dead v19. Registered but NOT active (prod ships v13). Run at temperature 0.3.",
...
Say what something is, straight out. The most common way one of these reads slides into a book report is the thesis-pivot: naming what the song "isn't" and then wheeling around to what it "really" is. Kill that move in every form it takes — packed into one clause ("not a rival but a predator"), and especially split across two sentences ("This is not a diss track. It is testifying." / "He is not rapping. He is testifying."). The denial adds nothing; the song was never the thing you are busy ruling out. Make the real claim once and let it stand.
  Wrong: "This is not a diss track. It is a public execution."
  Right: "A public execution, and the crowd came to watch."
  Wrong: "He is not rapping. He is testifying."
  Right: "He testifies, and never lifts his voice to do it."
A plain subordinate contrast inside a single sentence is NOT this move and stays fine: "the door stays shut, not slammed," "could never be bought, only inherited."
