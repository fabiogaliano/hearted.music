...
export const lyricalV15: PromptVersion = {
	version: "15",
	kind: "lyrical",
	notes:
		"Calibration iteration over v14 (Session 5.5 — first empirical contact). v14 was generated and measured for the first time (n=3 across the 10-song lyric diagnostic, temp 0.3): mean 2.14 high-severity Tier-1 hits per read, of which 87% were participial-closure (the comma-plus-'-ing' clause) leaking into arc scenes and line insights; the residual highs (self-reference, 'This is' openers, antithesis) and most puffery/copula mediums all concentrated in lines[].insight; arc and lines were pinned to the ceiling (arc=6 on 79% of reads, lines=5 on 64%, floors never used) even on a content-zero chant; ~1/3 of lenses drifted abstract ('happiness as precarity') with inconsistent casing. v15 changes, each aimed at a distinct measurable so the deltas stay attributable: (1) the comma-plus-'-ing' ban now covers the -ing-adjective case and carries arc-scene + line-insight examples plus a pre-finish self-check; (2) the lines insight and arc scene field specs reinforce the voice rules at the point of use, where critic-voice reverts; (3) cardinality is made structural, not maximal — arc length tracks the song's distinct sections and lines quote only distinct insights, with an explicit anti-padding clause; (4) the lens spec demands a picturable concrete Y and lowercase casing; (5) puffery adds adverb forms and the two leaked copula verbs (frames, acts as). Schema, lens grammar, and vocabulary are unchanged (locked Sessions 1-3); v14 is kept pristine as the comparison baseline. Intended to run at temperature 0.3, the proven low-variance setting.",
...
This is easiest to break inside an arc scene or a line insight, the two places it slips in most. Before you finish any sentence in any field, look at its last clause: if it opens with a comma and an "-ing" word, rewrite it.
...
**arc**: An array of 2 to 4 beats, one per genuinely distinct emotional turn the song makes, in sequence from open to outro. Count the song's real turns, not its sections: a track that cycles one chant or holds one mood is two or three beats. Most songs earn three. Reserve four for songs that truly travel through four distinct movements. Never pad to the maximum to look thorough. Each beat is an object with "label" (the section, in plain words: "Verse", "Pre Chorus", "Bridge"), "mood" (two or three words), and "scene" (one complete sentence that puts you inside that moment, with no comma followed by an "-ing" word). The mood may repeat across beats. A song in one emotional register has structure without changing register, and naming the same mood twice is honest. Do not manufacture movement that is not there.

**lines**: An array of 1 to 5 exact quotes from the lyrics, each an object with "line" (the quote) and "insight" (one complete sentence naming why it lands, insight first). Quote only the lines that each carry a distinct insight: a one-idea song earns one line, and padding to five to look thorough weakens the read. The insight is the field where critic-voice creeps back, so hold the line hardest here. No comma followed by an "-ing" word. Never open it with "This is" or "It is". Never write "the speaker" or "the listener". Never say what the line "is not" before saying what it is. No puffery ("profound"). For a line in another language, quote the original and follow it with a parenthetical English gloss: "Debí tirar más fotos (I should have taken more photos)".
