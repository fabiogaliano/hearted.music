...
export class LlmService {
	...
	private readonly languageModel: ReturnType<typeof createGoogleGenerativeAI> | ReturnType<typeof createAnthropic> | ReturnType<typeof createOpenAI>;
	...
	constructor(config: LlmConfig) {
		...
		switch (validated.provider) {
			case "google":
				this.languageModel = createGoogleGenerativeAI({ apiKey: validated.apiKey });
				break;
			case "anthropic":
	 ...
	 */
	private extractTokenUsage(usage: { inputTokens?: number; outputTokens?: number; totalTokens?: number } | undefined): TokenUsage | undefined {
	...
	private extractRetryAfter(error: Error): number | undefined {
		...
		if (typeof anyError.retryAfter === "number") return anyError.retryAfter * 1000;
