-- Deck read-model post-verification fix pass 3 (hardening): supersede
-- 20260707000019 by guarding the BRANCH-2 promotion-race re-read against a
-- NOT FOUND result.
--
-- When two writers race the one-active-per-orientation partial unique index,
-- the loser catches unique_violation and re-reads the winner's active session
-- to return the same view. Under PostgreSQL an INSERT that raises
-- unique_violation implies the winner COMMITTED (a loser blocked on an
-- uncommitted duplicate simply succeeds if the winner rolls back), so the
-- winner's row exists at the moment of the violation. The one remaining path
-- to a NOT FOUND re-read is narrow: a concurrent action flips the winner's
-- session out of status = 'active' (completed / abandoned) in the gap between
-- the violation and this re-read. This is not a reproduced live bug — it is
-- belt-and-suspenders hardening.
--
-- Without the guard, a NOT FOUND re-read leaves v_session all-NULL, v_created
-- false, so the promotion body is skipped and the SHARED block emits an
-- 'active' MatchDeckView carrying sessionId = NULL / orientation = NULL /
-- total = 0 — a malformed active deck rather than an honest miss. The added
-- three lines fall that case into the same miss the caller already handles via
-- its bounded poll re-read. Everything else is copied verbatim from
-- 20260707000019.
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    EXCEPTION WHEN unique_violation THEN
      ...
      ORDER BY s.created_at DESC
      LIMIT 1;
      -- Hardening: the unique_violation proves the winner committed an active
      -- row, but a concurrent completion/abandon can flip it out of 'active'
      -- before this re-read lands. If so, there is nothing to promote or
      -- resume — fall into the caller's miss path instead of emitting an
      -- 'active' view with a NULL sessionId.
      IF NOT FOUND THEN
        RETURN jsonb_build_object('status', 'miss', 'reason', 'no_ready_proposal');
      END IF;
      v_created := false;
