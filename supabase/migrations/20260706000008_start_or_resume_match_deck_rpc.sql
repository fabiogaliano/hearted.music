-- Match deck read model, Phase 1b (plan §8): start_or_resume_match_deck.
--
-- The one bounded request-path RPC for /match entry. Returns the full
-- MatchDeckView-shaped JSONB (plan §4) in one round trip, with a top-level
-- `status` discriminator the plan's view type does not carry:
--   status = 'active' — the full deck view (branches 1 and 2).
--   status = 'miss'   — no active session and no ready proposal for the latest
--                       snapshot+hash; the Phase 3 server fn does the bounded
--                       first-window build in TS. reason = 'no_ready_proposal'.
--
-- Three branches (plan §8):
--   1. Active session for (account, orientation) → return its view.
--   2. No active session but a READY proposal exists for the account's LATEST
--      snapshot + orientation + p_visibility_config_hash → promote in-RPC (create
--      session, bulk-insert queue items from the proposal subjects, copy the
--      promotion seed into visible pairs, write the snapshot ledger row, enqueue
--      a capture_ahead job), then return the same 'active' view.
--   3. Otherwise → miss. "No snapshot at all" is folded into the miss here; the
--      TS layer distinguishes it (plan §8).
--
-- VOLATILE: branch 2 writes, and both branches call read_match_deck_card (which
-- marks the current card presented). Current/next cards are built by calling
-- read_match_deck_card inline — p_mark_presented=true for CURRENT, false for
-- NEXT — so the deck view and the per-card read path share one code path.
...
BEGIN
  ...
  -- Progress counts over the session's items.
  SELECT count(*)
  INTO v_total
  ...
  SELECT COALESCE(jsonb_agg(qi.id ORDER BY qi.position ASC), '[]'::JSONB)
  INTO v_item_ids
