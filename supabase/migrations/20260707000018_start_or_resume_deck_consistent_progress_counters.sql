-- Deck read-model post-verification fix pass 2, P1.4: make start_or_resume_
-- match_deck's progress counters mutually consistent within one response.
--
-- The live body (20260706000013) computed v_total, v_remaining, and
-- v_item_ids as three independent SELECTs over match_review_queue_item.
-- Under READ COMMITTED each statement takes its own snapshot, so a
-- concurrent write between them (e.g. another session action resolving an
-- item) could make progress.remaining disagree with itemIds.length in the
-- same response. Fix: derive all three from one SELECT (single scan, single
-- snapshot) using FILTER-qualified aggregates instead of three separate
-- COUNT/jsonb_agg statements. Everything else is copied verbatim from
-- 20260706000013 (full H3 ON CONFLICT DO NOTHING bulk-insert bodies,
-- captured-empty seed stamp, security footer) — no other behavior changes.
