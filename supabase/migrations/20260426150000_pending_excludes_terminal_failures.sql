-- Make "pending" mean "still awaiting work", not "failed terminally".
--
-- get_liked_songs_stats.pending and get_liked_songs_page (p_filter='pending')
-- previously counted any entitled liked_song with no item_status row. After
-- 20260426000000_add_job_failure_metadata introduced is_terminal, songs that
-- have hit a terminal failure (validation / unsupported / auth / permanent /
-- … any future is_terminal=TRUE row) were still showing up as "pending" even
-- though no further processing will ever happen for them. That made the
-- pending bucket grow monotonically as failures accumulated and made the UI
-- progress signal misleading.
--
-- This migration tightens both selectors so terminal failures are excluded
-- from pending. The contract is otherwise unchanged: same return shape, same
-- filter names, same display_state values. Preprod-only clean break — no
-- failed bucket added to the UI surface.
...
BEGIN
  ...
  SELECT
    ...
    -- pending: entitled + no item_status + no terminal failure for this song.
    -- Terminal failures end the processing lifecycle, so they are not pending.
    COUNT(*) FILTER (
...
BEGIN
  ...
  WHERE ls.account_id = p_account_id
    ...
    AND (
      p_filter = 'all'
      -- pending: entitled, never actioned, and not terminally failed.
      OR (
        p_filter = 'pending'
        AND ist.id IS NULL
        AND ent.is_entitled
