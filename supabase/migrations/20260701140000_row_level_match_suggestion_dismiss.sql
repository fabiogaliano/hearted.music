-- Row-level dismiss for individual match suggestions.
--
-- Adds an atomic RPC that rejects one captured visible pair without resolving the
-- queue item. Existing whole-card finish/dismiss RPCs are updated so row-dismissed
-- pairs are not later re-labelled as skipped or dismissed again.
