-- Global server-side search for /liked-songs.
--
-- Extends get_liked_songs_page with an optional p_search argument that
-- matches against song.name and any element of song.artists (joined into a
-- single string). When p_search is NULL or whitespace, behavior is identical
-- to the previous function. Trigram GIN indexes back the ILIKE substring
-- match so search stays cheap as libraries grow.

CREATE EXTENSION IF NOT EXISTS pg_trgm WITH SCHEMA extensions;

-- array_to_string is STABLE, which prevents its direct use in an index
-- expression. Wrap it in an IMMUTABLE helper so we can both index and search
-- the joined artist string with the same expression.
CREATE OR REPLACE FUNCTION public.song_artists_joined(p_artists TEXT[])
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
...
AS $$
  SELECT array_to_string(COALESCE(p_artists, ARRAY[]::TEXT[]), ' ')
$$;

CREATE INDEX IF NOT EXISTS song_name_trgm_idx
  ON public.song
  USING gin (name extensions.gin_trgm_ops);

CREATE INDEX IF NOT EXISTS song_artists_joined_trgm_idx
  ON public.song
  USING gin (public.song_artists_joined(artists) extensions.gin_trgm_ops);

DROP FUNCTION IF EXISTS public.get_liked_songs_page(UUID, TIMESTAMPTZ, INTEGER, TEXT);

CREATE OR REPLACE FUNCTION public.get_liked_songs_page(
  p_account_id UUID,
...
BEGIN
  ...
  WHERE ls.account_id = p_account_id
    ...
    AND (
      ...
      OR s.name ILIKE v_search_pattern ESCAPE E'\\'
      OR public.song_artists_joined(s.artists) ILIKE v_search_pattern ESCAPE E'\\'
    )
    AND (
      p_filter = 'all'
      OR (
      ...
      OR (p_filter = 'analyzed' AND ent.is_entitled AND sa.id IS NOT NULL)
...
-- Re-apply hardening for the new signature. The 4-arg variant was dropped
-- above, so the previous REVOKE/GRANT in 20260519110000_harden_internal_rpcs
-- no longer targets a live function.
REVOKE EXECUTE ON FUNCTION
  public.get_liked_songs_page(UUID, TIMESTAMPTZ, INTEGER, TEXT, TEXT)
