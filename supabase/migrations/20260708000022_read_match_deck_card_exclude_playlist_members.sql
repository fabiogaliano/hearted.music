-- Deck read model fix: exclude songs already in the target playlist.
--
-- Bug: a song that is already a member of the reviewed playlist kept showing up
-- in SONG SUGGESTIONS. Membership exclusion previously lived ONLY at snapshot
-- write time (loadExclusionSet, enrichment-pipeline/stages/matching.ts), which:
--   (a) can miss rows — it re-enters a DB-derived playlist-id set as an .in()
--       URL filter with no error check, so a truncated/failed read silently
--       yields an incomplete (or empty) exclusion set; and
--   (b) is a point-in-time snapshot — membership is live state that can change
--       after the snapshot is written, so a write-time-only check can never be
--       the guarantee.
--
-- "Never suggest a song already in the playlist" is a hard invariant, so it is
-- enforced here at read time against the live playlist_song table — the same
-- layer that already excludes dismissed match_decision pairs. The predicate is
-- identical in both orientation arms because the visible_pair row (vp) carries
-- both song_id and playlist_id: exclude the pair when the suggestion song is
-- already in the (subject or suggestion) playlist.
--   playlist arm: drop suggestion SONGS already in the subject playlist.
--   song arm:     drop suggestion PLAYLISTS that already contain the subject song.
--
-- Full body copied verbatim from 20260706000014_read_match_deck_card_clear_newness;
-- only the two suggestion sub-selects gain the playlist_song NOT EXISTS predicate.
...
BEGIN
  ...
  IF v_item.orientation = 'playlist' THEN
    ...
    -- stable song_id ASC; dismissed pairs and current playlist members excluded
    -- in SQL.
  ...
  -- Playlists that already contain the subject song are excluded too — the
  -- mirror of the playlist arm's membership filter.
  SELECT COALESCE(jsonb_agg(row_data ORDER BY rn), '[]'::JSONB),
         COALESCE(MAX(total_ct), 0)
  INTO v_suggestions, v_total_active
  FROM (
