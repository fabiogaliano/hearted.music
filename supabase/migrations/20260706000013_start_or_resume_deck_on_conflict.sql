-- Deck read model fix (review H3, part b): promotion's two bulk INSERT ...
-- SELECT statements inside start_or_resume_match_deck trusted proposal/seed
-- rows verbatim. 20260706000012 makes malformed worker data fail at build
-- time going forward, but existing/in-flight bad rows (or any future gap in
-- the belt) must not turn "self-healing on miss" into a persistent 500 for
-- the account. Both bulk inserts get ON CONFLICT DO NOTHING; nothing else in
-- the function body changes.
--
-- Bare ON CONFLICT DO NOTHING (no target) on both statements: each target
-- table carries MULTIPLE unique constraints a duplicate could violate
-- (match_review_queue_item: idx_match_review_queue_item_session_position,
-- the per-orientation subject indexes, the per-orientation snapshot+subject
-- indexes; match_review_item_visible_pair: its PK (queue_item_id, song_id,
-- playlist_id) and idx_match_review_item_visible_pair_queue_visible_rank), so
-- no single conflict target is unambiguous.
--
-- Full body copied verbatim from 20260706000008_start_or_resume_match_deck_rpc.sql.
