-- Return the authoritative post-mutation credit balance from
-- unlock_songs_for_account so callers never derive the displayed balance from a
-- pre-RPC snapshot. The charge path already holds the row lock, so
-- (v_balance - v_cost) is the committed balance; the zero-cost path reads the
-- current balance directly since it performs no mutation. This eliminates the
-- stale-balance skew observable under concurrent unlocks (the DB was always
-- correct; only the number echoed back to the client could lag).
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'already_unlocked_song_ids', to_jsonb(v_already),
    'credit_balance',            v_balance - v_cost
  );
END;
$$;
