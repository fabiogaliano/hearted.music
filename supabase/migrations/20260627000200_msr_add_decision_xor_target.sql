-- MSR review fix (Finding 5): reject ambiguous suggestion targets in
-- add_match_review_item_decision_atomic.
--
-- The MSR-26 version validated only that the orientation's *expected* suggestion
-- side was non-null. It never rejected a call that supplied BOTH
-- p_suggestion_song_id and p_suggestion_playlist_id: for a song item the extra
-- p_suggestion_song_id was silently ignored (and vice versa for playlist items),
-- so an ambiguous call could write a decision the caller did not intend.
--
-- This re-creates the function with an explicit XOR guard before the orientation
-- branch: exactly one of the two suggestion IDs must be supplied. Both-null and
-- both-non-null now return 'invalid_target'. The per-orientation checks remain so
-- supplying the wrong single side is still rejected. Body is otherwise identical
-- to MSR-26; the argument list is unchanged so CREATE OR REPLACE is safe.
